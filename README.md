...
Gmail: Управление аккаунтом Google → Безопасность → Пароли приложений (нужна включённая 2FA)
...
Откройте https://api.telegram.org/bot<TOKEN>/getUpdates и найдите chat.id в ответе
🚀 Запуск
python3 main.py
...
Добавьте учётные данные в config.py
Добавьте запись в IMAP_PROVIDERS в main.py:
IMAP_PROVIDERS = {
...
Добавьте функцию-обёртку и вызов в цикле main():
